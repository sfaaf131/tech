...
export function Sidebar({ name, portal }: { name: string; portal: string }) {
  ...
  return (
    <aside className="border-b border-line bg-ink-2 md:min-h-screen md:w-64 md:border-r md:border-b-0">
      <div className="flex items-center justify-between px-5 py-4 md:block">
        <Link href="/dashboard" aria-label="Portal Kondax">
          <Logo />
        </Link>
        <p className="hidden font-mono text-[11px] tracking-[0.16em] text-copper uppercase md:mt-6 md:block">
          {portal === "startup" ? "Vista socio" : "Vista cliente B2B"}
        </p>
      </div>
      <nav className="flex gap-1 overflow-x-auto px-3 pb-3 md:flex-col md:px-3 md:pb-6" aria-label="Portal">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={cn(
              "rounded-xl px-3 py-2 text-sm whitespace-nowrap",
